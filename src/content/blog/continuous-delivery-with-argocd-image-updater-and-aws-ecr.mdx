...
Argo CD watches the GitHub repository and synchronizes it with the cluster for CD, while Jenkins handles CI. After a new image was pushed to ECR, the image tag in the repository containing the Kubernetes manifests still needed to be updated.
...
Argo CD Image Updater was still under active development, so it was not recommended for critical environments.
At the time of writing, v1 had not been released yet, and the latest version was v0.15.1.

This Continuous Delivery flow needed:
...
When the ECR repository is updated, the image tag in the GitHub repository is updated automatically.
...
Argo CD Image Updater connects ECR image updates to Git-based Kubernetes manifests without adding tag-update logic to Jenkins. For non-critical environments, it automates image-tag updates while Argo CD remains the deployment control point.
