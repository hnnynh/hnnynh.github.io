...
- ReLU, or Rectified Linear Unit, outputs `0` for negative inputs and leaves positive inputs unchanged. Because its output does not change by a constant ratio as the input changes, it is nonlinear.
...
- **Memory follows the total parameter count.** All expert weights must be available in GPU memory because the selected expert can vary by token.
